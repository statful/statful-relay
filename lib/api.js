...
module.exports = {
    ...
    processMetricLine: function (config) {
        ...
        var index_name = (config.es || {"index": "graphite_metrics2"}).index;
            ...
            if (es_info) {
                ...
                if (!client) {
                    client = new elasticsearch.Client({
                        host: es_info.host + ':' + es_info.port,
                        log: (config.es || {log: 'trace'}).log
                    });
                    clients[host] = client;
                }

                destinations[es_info.prefix] = {'host': es_info.host, 'port': es_info.port, 'client': client};
            }
        }
