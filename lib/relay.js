...
function processMetricLine(config, telemetron) {
    ...
    var processMetric = function (metric) {
        ...
        if (metric.agg) {
            ...
            commitMetric(metricToCommit, metric.prefix, l, telemetron);
        }
...
module.exports = {
    ...
    createServer: function (config, telemetron) {
        udp.start(config.udp.port, config.udp.address, config.udp.ipv6, processMetricLine(config, telemetron));
    }
};