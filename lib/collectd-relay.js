 ...
 * @returns {Function}
 */
function dispatchMetrics(config, telemetron, collectdApiSender) {
    return function (lines) {
        ...
        if (Array.isArray(metrics)) {
            ...
            if (config.autoDiagnostics && metrics.length > 0) {
...
CollectdRelay.prototype.start = function () {
    ...
    udp.start(udpConfig.port, udpConfig.address, udpConfig.ipv6,
        dispatchMetrics(this.config, this.telemetron, this.collectdApiSender));
};
