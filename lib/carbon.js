...
function processAndFlushAggregations(aggKey, l) {
    var metricsToCommit = [],
        ag = agg[aggKey],
    ...
    if (metricNames.length === 0) {
        ...
        commitMetric(metricsToCommit, ag.prefix, ag.destination, l);
    }


    //commitMetric(metricsToCommit, )
}
 ...
 * @param metric
 */
function aggregateMetric(metric, metricName, destination, l) {
    ...
    if (!agg[aggKey]) {
        agg[aggKey] = {
            ...
            timer: setInterval(processAndFlushAggregations, f * 1000, aggKey, l)
        };
 ...
 * @param l
 */
function commitMetric(metrics, prefix, destination, l) {
...
function processMetricLine(config) {
    ...
    var processMetric = function (metric) {
        ...
        var metricName = [metric.prefix, metric.name].join('.'),
            metricToCommit;
        ...
        if (metric.agg) {
            aggregateMetric(metric, metricName, config.destinations[metric.prefix], l);
        } else {
            metricToCommit = [{
                'name': metricName,
                'columns': ['time', 'value'],
            ...
            commitMetric(metricToCommit, metric.prefix, config.destinations[metric.prefix], l);
        }
